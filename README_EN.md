...
cloning, point the setup script at your own UE 5.8 installation; it creates the
virtual environment and builds a local lexical/symbol index:
...
Start the MCP server for a local AI client:

```powershell
.\scripts\start_mcp.ps1
```

Double-click `启动RAG数据管理台.bat` in the repository root to open the local
data manager. It lets users select an Unreal project, preview added, modified,
and deleted files, then synchronize lexical and optional vector indexes without
using a command line.

To generate a read-only static dashboard instead:

```powershell
python scripts/visualize.py --open
```

The self-contained report is written to `outputs/rag_dashboard.html`. It shows
corpus composition, chunk sizes, index readiness, vector coverage, and benchmark
metrics. Only aggregate statistics are included; source and documentation bodies
are never copied into the dashboard.

See the [GitHub clone and usage guide](docs/09_GitHub使用指南.md) for client configuration,
LAN access, semantic-search setup, and repository publishing notes.
...
The project implements shared data contracts, documentation and Unreal source
ingestion, semantic chunking, lexical and vector indexing, hybrid retrieval,
reranking, evaluation, and MCP integration.
...
Embedding and reranking model dependencies are optional because they are much
larger than the lexical-only runtime:
...
- `config/embedding.yaml` — embedding provider and model settings
- `config/retrieval.yaml` — retrieval and reranking limits
- `.env.example` — environment-specific values; copy it to `.env` when needed

Set `UE_ROOT` to the local Unreal Engine installation before running ingestion.
The setup script does this automatically for its own process. Do not commit a
machine-specific engine path, a local `.env` file, or generated Unreal Engine
data.
...
required. HTTP bind address and port can be changed with `--host` and `--port`.
...
The T01–T20 implementation chain is complete. A first 25-question Hybrid
benchmark is documented in
[the benchmark guide](docs/08_Benchmark评测指南.md); the next quality milestone
is a reviewed 100-question benchmark followed by retrieval and ranking tuning.

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) for development setup and repository
safety rules. Generated Unreal Engine data, indexes, model caches, private
project source, and credentials must not be committed.
